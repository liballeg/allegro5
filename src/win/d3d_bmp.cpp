...
static A5O_BITMAP_INTERFACE *vt;
...
static bool convert_compressed(LPDIRECT3DTEXTURE9 dest, LPDIRECT3DTEXTURE9 src,
   ...
   if (src_texture_surface) {
       if ((i = src_texture_surface->Release()) != 0) {
          A5O_DEBUG("convert_compressed (src) ref count == %d\n", i);
       }
   }
   if (dest_texture_surface) {
       if ((i = dest_texture_surface->Release()) != 0) {
          // This can be non-zero
          A5O_DEBUG("convert_compressed (dest) ref count == %d\n", i);
       }
   }
   return ok;
#else
 ...
 */
bool al_get_d3d_texture_size(A5O_BITMAP *bitmap, int *width, int *height)
{
   ...
   if (!(bitmap_flags & _A5O_INTERNAL_OPENGL) &&
...
static void d3d_draw_textured_quad(
   A5O_DISPLAY_D3D *disp, A5O_BITMAP *bmp, A5O_COLOR tint,
   float sx, float sy, float sw, float sh, int flags)
{
   ...
   int texture_w;
   int texture_h;
   A5O_BITMAP_EXTRA_D3D *extra = get_extra(bmp);

   const float z = 0.0f;
   
   A5O_DISPLAY* aldisp = (A5O_DISPLAY*)disp;
...
#define A5O_COLOR_TO_D3D(c) D3DCOLOR_COLORVALUE(c.r, c.g, c.b, c.a)
...
\
   ...
   bool pp = (aldisp->flags & A5O_PROGRAMMABLE_PIPELINE) != 0;

   if (pp) {
           A5O_VERTEX *vertices = (A5O_VERTEX *)aldisp->vt->prepare_vertex_cache(aldisp, 6);
        SET(A5O_COLOR)
   }
   else {
           D3D_FIXED_VERTEX *vertices = (D3D_FIXED_VERTEX *)aldisp->vt->prepare_vertex_cache(aldisp, 6);
        SET(A5O_COLOR_TO_D3D)
   }
...
static void d3d_sync_bitmap_memory(A5O_BITMAP *bitmap)
{
...
static void d3d_sync_bitmap_texture(A5O_BITMAP *bitmap,
   int x, int y, int width, int height)
{
   D3DLOCKED_RECT locked_rect;
   RECT rect;
...
static void d3d_do_upload(A5O_BITMAP *bmp, int x, int y, int width,
   int height, bool sync_from_memory)
{
   ...
   if (_al_d3d_render_to_texture_supported()
      ...
      A5O_BITMAP_EXTRA_D3D *d3d_bitmap = get_extra(bmp);
 ...
 */
void _al_d3d_release_default_pool_textures(A5O_DISPLAY *disp)
{
   unsigned int i;

   for (i = 0; i < disp->bitmaps._size; i++) {
      A5O_BITMAP **bptr = (A5O_BITMAP **)_al_vector_ref(&disp->bitmaps, i);
      A5O_BITMAP *albmp = *bptr;

      if ((al_get_bitmap_flags(albmp) & A5O_MEMORY_BITMAP) || (albmp->parent))
         continue;

      A5O_BITMAP_EXTRA_D3D *d3d_bmp = get_extra(albmp);
...
static bool d3d_create_textures(A5O_DISPLAY_D3D *disp, int w, int h,
   int flags,
...
static bool _al_d3d_sync_bitmap(A5O_BITMAP *dest)
{
   A5O_BITMAP_EXTRA_D3D *d3d_dest;
   ...
   if (system_texture_surface) {
       if ((i = system_texture_surface->Release()) != 0) {
          A5O_DEBUG("_al_d3d_sync_bitmap (system) ref count == %d\n", i);
       }
   }
   if (video_texture_surface) {
       if ((i = video_texture_surface->Release()) != 0) {
          // This can be non-zero
          A5O_DEBUG("_al_d3d_sync_bitmap (video) ref count == %d\n", i);
       }
   }
...
static void d3d_backup_dirty_bitmap(A5O_BITMAP *bitmap)
{
   A5O_DISPLAY *display = bitmap->_display;
   ...
   A5O_BITMAP_EXTRA_D3D *extra = get_extra(bitmap);
   int bitmap_flags = al_get_bitmap_flags(bitmap);
   if (!(
      (bitmap_flags & A5O_MEMORY_BITMAP) ||
...
{
   unsigned int i;
   A5O_DISPLAY *display = (A5O_DISPLAY *)disp;
...
void _al_d3d_refresh_texture_memory(A5O_DISPLAY *display)
{
   ...
   for (i = 0; i < display->bitmaps._size; i++) {
      ...
      A5O_BITMAP *bmp = *bptr;
      A5O_BITMAP_EXTRA_D3D *extra = get_extra(bmp);
      A5O_DISPLAY_D3D *bmps_display =
         (A5O_DISPLAY_D3D *)_al_get_bitmap_display(bmp);
      int bitmap_flags = al_get_bitmap_flags(bmp);

      if ((bitmap_flags & A5O_MEMORY_BITMAP) || bmp->parent) {
         continue;
      }
...
static bool d3d_upload_bitmap(A5O_BITMAP *bitmap)
{
   A5O_BITMAP_EXTRA_D3D *d3d_bmp = get_extra(bitmap);
   ...
   float sx, float sy, float sw, float sh, int flags)
{
   ...
   A5O_BITMAP_EXTRA_D3D *d3d_dest = get_extra(dest);
   A5O_BITMAP_EXTRA_D3D *d3d_src = get_extra(src);
...
static A5O_LOCKED_REGION *d3d_lock_region(A5O_BITMAP *bitmap,
   ...
   int flags)
{
   ...
   RECT rect;
   DWORD Flags = flags & A5O_LOCK_READONLY ? D3DLOCK_READONLY : 0;
   ...
   if (d3d_bmp->is_backbuffer) {
      A5O_DISPLAY_D3D *d3d_disp =
         (A5O_DISPLAY_D3D *)_al_get_bitmap_display(bitmap);
      ...
      if (_al_pixel_format_is_compressed(bitmap_format)) {
          ...
          */
         bitmap->locked = false;
         if (!(flags & A5O_LOCK_WRITEONLY) && !_al_d3d_sync_bitmap(bitmap)) {
            return NULL;
         }
         bitmap->locked = true;
         texture = d3d_bmp->system_texture;
   ...
   if (format == A5O_PIXEL_FORMAT_ANY || system_format == format || system_format == f) {
...
static void d3d_unlock_region(A5O_BITMAP *bitmap)
{
   A5O_BITMAP_EXTRA_D3D *d3d_bmp = get_extra(bitmap);
   int system_format = d3d_bmp->system_format;
...
static void d3d_update_clipping_rectangle(A5O_BITMAP *bitmap)
{
   _al_d3d_set_bitmap_clip(bitmap);
}


static A5O_LOCKED_REGION *d3d_lock_compressed_region(
   A5O_BITMAP *bitmap, int x, int y, int w, int h,
   int flags)
{
   A5O_BITMAP_EXTRA_D3D *d3d_bmp = get_extra(bitmap);
   int bitmap_format = al_get_bitmap_format(bitmap);
...
static void d3d_unlock_compressed_region(A5O_BITMAP *bitmap)
{
   A5O_BITMAP_EXTRA_D3D *d3d_bmp = get_extra(bitmap);
...
A5O_BITMAP_INTERFACE *_al_bitmap_d3d_driver(void)
{
   ...
   vt = (A5O_BITMAP_INTERFACE *)al_malloc(sizeof *vt);
