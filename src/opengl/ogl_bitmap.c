    ...
    int flags)
{
   ...
   _AL_BATCH_INDEX_TYPE first_idx;
   bool use_indices = disp->batch_use_indices;
   if (use_indices)
   ...
   if (disp->cache_enabled) {
      ...
      if (use_indices)
         ...
         transform_vertex(&vtx[4].x, &vtx[4].y, &vtx[4].z);
   }
   ...
   float sw, float sh, int flags)
{
   ...
   if (disp->ogl_extras->opengl_target == target) {
      if (disp->use_legacy_drawing_api)
         ...
         draw_quad_new(disp, bitmap, tint, sx, sy, sw, sh, flags);
      return;
   }
