   ...
   int w, int h, int format, int refresh_rate)
{
   ...
   int n = _al_xdummy_get_num_display_modes(s);
   if (!n) return false;
   ...
   for (i = 0; i < n; i++) {
      _al_xdummy_get_display_mode(s, i, &mode);
   ...
   _al_xdummy_get_display_mode(s, best_mode, &mode);
   for (i = 1; i < possible_count; i++) {
      _al_xdummy_get_display_mode(s, possible_modes[i], &mode2);
...
{
   _al_xdummy_get_num_display_modes(s);
...
void _al_xdummy_store_video_mode(void)
{
}

void _al_xdummy_restore_video_mode(void)
{
}
#endif
