...
typedef struct A5O_MOUSE AL_MOUSE;
typedef struct A5O_MOUSE_STATE AL_MOUSE_STATE;
...
{
   memset(&osx_mouse.state, 0, sizeof(A5O_MOUSE_STATE));
}

/* _al_osx_switch_keyboard_focus:
 *  Handle a focus switch event.
 */
void _al_osx_switch_mouse_focus(A5O_DISPLAY *dpy, bool switch_in)
{
...
static A5O_MOUSE* osx_get_mouse(void)
{
   return (A5O_MOUSE*) &osx_mouse.parent;
}
...
*/
void _al_osx_mouse_generate_event(NSEvent* evt, A5O_DISPLAY* dpy)
{
   NSPoint pos;
   ...
   switch ([evt type])
   {
      ...
      case NSOtherMouseDown:
         type = A5O_EVENT_MOUSE_BUTTON_DOWN;
         b = [evt buttonNumber]+1;
         b_change = 1;
      ...
      case NSRightMouseUp:
      case NSOtherMouseUp:
         type = A5O_EVENT_MOUSE_BUTTON_UP;
         b = [evt buttonNumber]+1;
         b_change = 1;
         osx_mouse.state.buttons &= ~(1 << (b-1));
         pressure = [evt pressure];
      ...
      case NSScrollWheel:
         ...
         break;
      case NSMouseEntered:
         type = A5O_EVENT_MOUSE_ENTER_DISPLAY;
         b = [evt buttonNumber]+1;
         dx = [evt deltaX];
         dy = [evt deltaY];
         break;
   ...
   dx *= scaling_factor;
   dy *= scaling_factor;
   if (osx_mouse.warped && type == A5O_EVENT_MOUSE_AXES) {
   ...
   {
      A5O_EVENT new_event;
      A5O_MOUSE_EVENT* mouse_event = &new_event.mouse;
...
static bool osx_init_mouse(void)
{
   ...
   A5O_INFO("Detected %d pointing devices\n", devices.count);
   for (i=0; i<devices.count; i++) {
      ...
      A5O_INFO("Detected %d axes and %d buttons\n", axes, buttons);
   ...
   A5O_INFO("Device effectively has %d axes and %d buttons\n", axes, buttons);
...
static void osx_get_mouse_state(A5O_MOUSE_STATE *ret_state)
{
   _al_event_source_lock(&osx_mouse.parent.es);
   memcpy(ret_state, &osx_mouse.state, sizeof(A5O_MOUSE_STATE));
   _al_event_source_unlock(&osx_mouse.parent.es);
}
...
*/
static bool osx_set_mouse_xy(A5O_DISPLAY *dpy_, int x, int y)
{
   CGPoint pos;
   CGDirectDisplayID display = 0;
   A5O_DISPLAY_OSX_WIN *dpy = (A5O_DISPLAY_OSX_WIN *)dpy_;
   float scaling_factor = 1.0;
#if MAC_OS_X_VERSION_MIN_REQUIRED >= 1070
   scaling_factor = [[NSScreen mainScreen] backingScaleFactor];
#endif
   ...
   if ((dpy) && !(dpy->parent.flags & A5O_FULLSCREEN) &&
         !(dpy->parent.flags & A5O_FULLSCREEN_WINDOW) && (dpy->win)) {
   ...
   if (_al_event_source_needs_to_generate_event(&osx_mouse.parent.es)) {
      A5O_EVENT new_event;
      A5O_MOUSE_EVENT* mouse_event = &new_event.mouse;
      mouse_event->type = A5O_EVENT_MOUSE_WARPED;
      // Note: we use 'allegro time' rather than the time stamp
      // from the event
      mouse_event->timestamp = al_get_time();
      mouse_event->display = (A5O_DISPLAY *)dpy;
...
/* Mouse driver */
static A5O_MOUSE_DRIVER osx_mouse_driver =
{
   0, //int  msedrv_id;
   ...
   osx_get_mouse, //AL_METHOD(A5O_MOUSE*, get_mouse, (void));
   ...
   osx_get_mouse_state, //AL_METHOD(void, get_mouse_state, (A5O_MOUSE_STATE *ret_state));
};
A5O_MOUSE_DRIVER* _al_osx_get_mouse_driver(void)
{
