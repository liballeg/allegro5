...
static const unsigned int allegro_to_osx_settings[][3] = {
   { A5O_RED_SIZE,           0, 0},   // Not supported per component
   { A5O_GREEN_SIZE,         0, 0},   // Not supported per component
   { A5O_BLUE_SIZE,          0, 0},   // Not supported per component
   { A5O_ALPHA_SIZE,         NSOpenGLPFAAlphaSize, 1},
   { A5O_RED_SHIFT,          0, 0},   // Not available
   { A5O_GREEN_SHIFT,        0, 0},   // Not available
   { A5O_BLUE_SHIFT,         0, 0},   // Not available
   { A5O_ALPHA_SHIFT,        0, 0},   // Not available
   { A5O_ACC_RED_SIZE,       NSOpenGLPFAAccumSize, 1}, // Correct?
   { A5O_ACC_GREEN_SIZE,     NSOpenGLPFAAccumSize, 1}, // Correct?
   { A5O_ACC_BLUE_SIZE,      NSOpenGLPFAAccumSize, 1}, // Correct?
   { A5O_ACC_ALPHA_SIZE,     NSOpenGLPFAAccumSize, 1}, // Correct?
   { A5O_STEREO,             NSOpenGLPFAStereo, 0},
   { A5O_AUX_BUFFERS,        NSOpenGLPFAAuxBuffers, 1},
   { A5O_COLOR_SIZE,         NSOpenGLPFAColorSize, 1},
   { A5O_DEPTH_SIZE,         NSOpenGLPFADepthSize, 1},
   { A5O_STENCIL_SIZE,       NSOpenGLPFAStencilSize, 1},
   { A5O_SAMPLE_BUFFERS,     NSOpenGLPFASampleBuffers, 1},
   { A5O_SAMPLES,            NSOpenGLPFASamples, 1},
   //{ A5O_RENDER_METHOD,      NSOpenGLPFAAccelerated, 0}, handled separately
   { A5O_FLOAT_COLOR,        NSOpenGLPFAColorFloat, 0},
   { A5O_FLOAT_DEPTH,        0, 0},
   //{ A5O_SINGLE_BUFFER ,     0, 0}, handled separately
   { A5O_SWAP_METHOD,        0, 0},
   { A5O_COMPATIBLE_DISPLAY, 0, 0},
   { A5O_DISPLAY_OPTIONS_COUNT, 0, 0}
};
...
static const char *allegro_pixel_format_names[] = {
   ...
   "A5O_SINGLE_BUFFER",
   "A5O_SWAP_METHOD",
   "A5O_COMPATIBLE_DISPLAY",
   "A5O_DISPLAY_OPTIONS_COUNT"
};

/* Module functions */
A5O_DISPLAY_INTERFACE* _al_osx_get_display_driver(void);
A5O_DISPLAY_INTERFACE* _al_osx_get_display_driver_win(void);
A5O_DISPLAY_INTERFACE* _al_osx_get_display_driver_fs(void);
static NSOpenGLContext* osx_create_shareable_context(NSOpenGLPixelFormat* fmt, unsigned int* group);
static bool set_display_flag(A5O_DISPLAY *display, int flag, bool onoff);
static bool resize_display_win(A5O_DISPLAY *d, int w, int h);
static bool resize_display_win_main_thread(A5O_DISPLAY *d, int w, int h);
 ...
 */
static void _al_osx_change_cursor(A5O_DISPLAY_OSX_WIN *dpy, NSCursor* cursor)
{
...
-(void)setAllegroDisplay: (A5O_DISPLAY*) ptr;
-(A5O_DISPLAY*) allegroDisplay;
-(void) reshape;
...
-(void) zoom:(id)sender
{
   self.display->flags ^= A5O_MAXIMIZED;
   [super zoom:sender];
}
...
void _al_osx_mouse_was_installed(BOOL install) {
   ...
   dispatch_sync(dispatch_get_main_queue(), ^{
      ...
      for (i = 0; i < _al_vector_size(dpys); ++i) {
         A5O_DISPLAY_OSX_WIN* dpy = *(A5O_DISPLAY_OSX_WIN**) _al_vector_ref(dpys, i);
...
- (void) reshape
{
   ...
   A5O_DISPLAY_OSX_WIN* dpy =  (A5O_DISPLAY_OSX_WIN*) dpy_ptr;
   if (dpy->tracking) {
...
{
   A5O_DISPLAY_OSX_WIN* dpy =  (A5O_DISPLAY_OSX_WIN*) dpy_ptr;
   A5O_EVENT_SOURCE* src = &([self allegroDisplay]->es);
   if (dpy->show_cursor) {
      [dpy->cursor set];
   }
   else {
...
-(void) viewDidChangeBackingProperties
{
   ...
   if (!(al_get_display_flags(dpy_ptr) & A5O_RESIZABLE) &&
       dpy_ptr->ogl_extras) {
      ...
      A5O_EVENT_SOURCE *es = &dpy->parent.es;
...
-(void) windowDidBecomeMain:(NSNotification*) notification
{
   ...
   _al_event_source_lock(src);
   A5O_EVENT evt;
   evt.type = A5O_EVENT_DISPLAY_SWITCH_IN;
   _al_event_source_emit_event(src, &evt);
...
-(void) windowDidResignMain:(NSNotification*) notification
{
   ...
   _al_event_source_lock(src);
   A5O_EVENT evt;
   evt.type = A5O_EVENT_DISPLAY_SWITCH_OUT;
   _al_event_source_emit_event(src, &evt);
...
-(void) windowDidResize:(NSNotification*) notification
{
   ...
   if (_al_event_source_needs_to_generate_event(es)) {
      ...
      event.display.height = NSHeight(content);
      _al_event_source_emit_event(es, &event);
      A5O_INFO("Window was resized %d x %d\n", event.display.width, event.display.height);
   }
   _al_event_source_unlock(es);
}
   ...
   defaultFrame: (NSRect) newFrame
{
   ...
   if (dpy_ptr->use_constraints) {
      if (dpy_ptr->flags & A5O_MAXIMIZED) {
...
-(void) exitFullScreenWindowMode
{
   ...
   if (!(dpy_ptr->flags & A5O_FRAMELESS)) {
      [dpy->win setStyleMask: [dpy->win styleMask] | NSWindowStyleMaskTitled];
   }
}
...
*/
static bool set_current_display(A5O_DISPLAY* d) {
   A5O_DISPLAY_OSX_WIN* dpy = (A5O_DISPLAY_OSX_WIN*) d;
   if (dpy->ctx != nil) {
 ...
 */
static void osx_set_opengl_pixelformat_attributes(A5O_DISPLAY_OSX_WIN *dpy)
{
   int i, n;
    ...
    * requested.
    */
   const int sample_flags = (1<<A5O_SAMPLES)|(1<<A5O_SAMPLE_BUFFERS);
    ...
    */
   if (dpy->parent.flags & A5O_FULLSCREEN) {
   ...
   if (extras)
      dpy->depth = extras->settings[A5O_COLOR_SIZE];
   if (!dpy->depth) {   /* Use default */
      ...
      if (dpy->depth == 15)
         dpy->depth = 16;
      A5O_DEBUG("Using default colour depth %d\n", dpy->depth);
   }
   ...
   *a = dpy->depth; a++;
    ...
    */
   if (extras && (extras->required & (1 << A5O_SINGLE_BUFFER)) &&
         extras->settings[A5O_SINGLE_BUFFER]) {
   ...
   if (extras && (extras->required & sample_flags) &&
       (extras->settings[A5O_SAMPLES]||extras->settings[A5O_SAMPLE_BUFFERS])) {
    ...
    * for suggested.
    */
   if (extras->required & A5O_RENDER_METHOD) {
 ...
 * we have selected.
 */
static void osx_get_opengl_pixelformat_attributes(A5O_DISPLAY_OSX_WIN *dpy)
{
   int n;
   ...
   for (n = 0; n < number_of_settings; n++) {
      ...
      A5O_DEBUG("Pixel format attribute %s set to %d\n", allegro_pixel_format_names[n], value);
   }
#else
      ...
      while (*a) {
         if (*a == allegro_to_osx_settings[n][1]) {
            ...
            if (allegro_to_osx_settings[n][2])
               value = a[1];
            dpy->parent.extra_settings.settings[al_setting] = value;
            A5O_DEBUG("Setting pixel format attribute %d to %d\n", al_setting, value);
         }
...
#endif
   dpy->parent.extra_settings.settings[A5O_COMPATIBLE_DISPLAY] = 1;

   // Fill in the missing colour format options, as best we can
   A5O_EXTRA_DISPLAY_SETTINGS *eds = &dpy->parent.extra_settings;
   if (eds->settings[A5O_COLOR_SIZE] == 0) {
      ...
      int size = eds->settings[A5O_ALPHA_SIZE];
      if (!size) {
         switch (eds->settings[A5O_COLOR_SIZE]) {
            case 32:
               size = 8;
               break;
      ...
      if (!eds->settings[A5O_RED_SIZE])
         eds->settings[A5O_RED_SIZE] = size;
      if (!eds->settings[A5O_BLUE_SIZE])
         eds->settings[A5O_BLUE_SIZE] = size;
      if (!eds->settings[A5O_GREEN_SIZE])
         eds->settings[A5O_GREEN_SIZE] = size;
      if (!eds->settings[A5O_RED_SHIFT])
         eds->settings[A5O_RED_SHIFT] = 0;
      if (!eds->settings[A5O_GREEN_SHIFT])
         eds->settings[A5O_GREEN_SHIFT] = eds->settings[A5O_RED_SIZE];
      if (!eds->settings[A5O_BLUE_SHIFT])
         eds->settings[A5O_BLUE_SHIFT] = eds->settings[A5O_GREEN_SIZE]+eds->settings[A5O_GREEN_SHIFT];
      if (!eds->settings[A5O_ALPHA_SHIFT])
         eds->settings[A5O_ALPHA_SHIFT] = eds->settings[A5O_BLUE_SIZE]+eds->settings[A5O_BLUE_SHIFT];
   }
}

/* This function must be run on the main thread */
static void osx_run_fullscreen_display(A5O_DISPLAY_OSX_WIN* dpy)
{
   ASSERT_MAIN_THREAD();
   A5O_DISPLAY* display = &dpy->parent;
...
static NSOpenGLContext* osx_create_shareable_context(NSOpenGLPixelFormat* fmt, unsigned int* group)
{
   ...
   for (i = 0; i < _al_vector_size(dpys); ++i) {
      A5O_DISPLAY_OSX_WIN* other = *(A5O_DISPLAY_OSX_WIN**) _al_vector_ref(dpys, i);
      ...
      // OK, we can share with this one
         *group = other->display_group;
         A5O_DEBUG("Sharing display group %d\n", *group);
         break;
      }
   }
   if (compat == nil) {
      ...
      *group = next_display_group++;
      A5O_DEBUG("Creating new display group %d\n", *group);
                                      ...
                                      void *user_info)
{
   ...
   (void)flags_in;
   (void)flags_out;
   A5O_DISPLAY_OSX_WIN *dpy = (A5O_DISPLAY_OSX_WIN*)user_info;
...
static void init_new_vsync(A5O_DISPLAY_OSX_WIN *dpy)
{
...
static void init_halt_events(A5O_DISPLAY_OSX_WIN *dpy)
{
 ...
 */
static A5O_DISPLAY* create_display_fs(int w, int h)
{
   NSAutoreleasePool *pool = [[NSAutoreleasePool alloc] init];
   A5O_DEBUG("Switching to fullscreen mode sized %dx%d\n", w, h);
   ...
   A5O_DISPLAY_OSX_WIN* dpy = al_malloc(sizeof(A5O_DISPLAY_OSX_WIN));
   if (dpy == NULL) {
      [pool drain];
      return NULL;
   }
   ...
   display->ogl_extras = al_malloc(sizeof(A5O_OGL_EXTRAS));
   memset(display->ogl_extras, 0, sizeof(A5O_OGL_EXTRAS));
   ...
   if (_al_get_new_display_settings()->settings[A5O_VSYNC] == 1) {
      init_new_vsync(dpy);
   }
#else
   if (_al_get_new_display_settings()->settings[A5O_VSYNC] == 1) {
   ...
   A5O_DISPLAY **add = _al_vector_alloc_back(&al_get_system_driver()->displays);
   *add = display;
...
static A5O_DISPLAY* create_display_fs(int w, int h)
{
   A5O_DEBUG("Creating full screen mode sized %dx%d\n", w, h);
   if (al_get_new_display_adapter() >= al_get_num_video_adapters())
      return NULL;
   A5O_DISPLAY_OSX_WIN* dpy = al_malloc(sizeof(A5O_DISPLAY_OSX_WIN));
   if (dpy == NULL) {
   ...
   dpy->parent.flags = al_get_new_display_flags() | A5O_OPENGL | A5O_FULLSCREEN;
#ifdef A5O_CFG_OPENGLES2
   dpy->parent.flags |= A5O_PROGRAMMABLE_PIPELINE;
#endif
#ifdef A5O_CFG_OPENGLES
   dpy->parent.flags |= A5O_OPENGL_ES_PROFILE;
#endif
   dpy->parent.w = w;
   dpy->parent.h = h;
   ...
   // Set up the Allegro OpenGL implementation
   dpy->parent.ogl_extras = al_malloc(sizeof(A5O_OGL_EXTRAS));
   memset(dpy->parent.ogl_extras, 0, sizeof(A5O_OGL_EXTRAS));
   _al_ogl_manage_extensions(&dpy->parent);
   _al_ogl_set_extensions(dpy->parent.ogl_extras->extension_api);
...
*/
static A5O_DISPLAY* create_display_win(int w, int h) {
   ASSERT_USER_THREAD();
   NSAutoreleasePool *pool = [[NSAutoreleasePool alloc] init];
   /* Create a temporary view so that we can check whether a fullscreen
    * window can be created.
    */
   if (al_get_new_display_flags() & A5O_FULLSCREEN_WINDOW) {
      __block BOOL ok;
      dispatch_sync(dispatch_get_main_queue(), ^{
      ...
      if (!ok) {
         A5O_DEBUG("Cannot create FULLSCREEN_WINDOW");
         return NULL;
      }
   }

   A5O_DEBUG("Creating window sized %dx%d\n", w, h);
   if (al_get_new_display_adapter() >= al_get_num_video_adapters()) {
      [pool drain];
      return NULL;
   }
   ...
   display->refresh_rate = al_get_new_display_refresh_rate();
   display->flags = al_get_new_display_flags() | A5O_OPENGL | A5O_WINDOWED;
#ifdef A5O_CFG_OPENGLES2
   display->flags |= A5O_PROGRAMMABLE_PIPELINE;
#endif
#ifdef A5O_CFG_OPENGLES
   ...
   dispatch_sync(dispatch_get_main_queue(), ^{
      ...
      NSWindow* win = alwin;
      NSScreen *screen;
      unsigned int mask = (display->flags & A5O_FRAMELESS) ? NSWindowStyleMaskBorderless :
   ...
   *add = display;
   ...
   if (display->flags & A5O_FULLSCREEN_WINDOW) {
      display->flags ^= A5O_FULLSCREEN_WINDOW; /* Not set yet */
      set_display_flag(display, A5O_FULLSCREEN_WINDOW, true);
   }
 ...
 */
static void destroy_display(A5O_DISPLAY* d)
{
   ASSERT_USER_THREAD();
   NSAutoreleasePool *pool = [[NSAutoreleasePool alloc] init];
   A5O_DISPLAY *old_dpy = al_get_current_display();
   A5O_DISPLAY_OSX_WIN* dpy = (A5O_DISPLAY_OSX_WIN*) d;
   A5O_DISPLAY_OSX_WIN* other = NULL;
   unsigned int i;
   ...
   _AL_VECTOR* dpys = &al_get_system_driver()->displays;
   for (i = 0; i < _al_vector_size(dpys); ++i) {
      A5O_DISPLAY_OSX_WIN* d = *(A5O_DISPLAY_OSX_WIN**) _al_vector_ref(dpys, i);
   ...
   if (other != NULL) {
      ...
      for (i = 0; i<_al_vector_size(bmps); ++i) {
         A5O_BITMAP **add = _al_vector_alloc_back(&other->parent.bitmaps);
         A5O_BITMAP **ref = _al_vector_ref(bmps, i);
         *add = *ref;
      ...
      while (dpy->parent.bitmaps._size > 0) {
         A5O_BITMAP **bptr = _al_vector_ref_back(&dpy->parent.bitmaps);
         A5O_BITMAP *bmp = *bptr;
   ...
   A5O_DISPLAY* display = &dpy->parent;
   A5O_OGL_EXTRAS *ogl = display->ogl_extras;
   ...
   dispatch_sync(dispatch_get_main_queue(), ^{
      ...
      else if (display->flags & A5O_FULLSCREEN_WINDOW) {
#if MAC_OS_X_VERSION_MIN_REQUIRED >= 1050
         if (dpy->win) {
            [dpy->view exitFullScreenModeWithOptions: nil];
         }
 ...
 * Call from user thread
 */
static A5O_DISPLAY* create_display(int w, int h)
{
   ASSERT_USER_THREAD();
...
static void flip_display(A5O_DISPLAY *disp)
{
   A5O_DISPLAY_OSX_WIN* dpy = (A5O_DISPLAY_OSX_WIN*) disp;

   A5O_BITMAP *old_target = NULL;
   if (!((A5O_BITMAP_EXTRA_OPENGL *)disp->ogl_extras->opengl_target->extra)->is_backbuffer) {
...
static void update_display_region(A5O_DISPLAY *disp,
 ...
 */
A5O_MOUSE_CURSOR *_al_osx_create_mouse_cursor(A5O_BITMAP *bmp,
   int x_focus, int y_focus)
{
   NSAutoreleasePool *pool = [[NSAutoreleasePool alloc] init];
   A5O_MOUSE_CURSOR_OSX *cursor = NULL;
...
{
   A5O_MOUSE_CURSOR_OSX *cursor = (A5O_MOUSE_CURSOR_OSX *) curs;
   unsigned i;
   ...
   for (i = 0; i < _al_vector_size(dpys); ++i) {
      ...
      A5O_DISPLAY_OSX_WIN *osx_dpy = (A5O_DISPLAY_OSX_WIN*) dpy;
 ...
 */
static bool osx_set_mouse_cursor(A5O_DISPLAY *display,
   A5O_MOUSE_CURSOR *cursor)
{
   A5O_DISPLAY_OSX_WIN *dpy = (A5O_DISPLAY_OSX_WIN *)display;
   A5O_MOUSE_CURSOR_OSX *osxcursor = (A5O_MOUSE_CURSOR_OSX *)cursor;
 ...
 */
static bool osx_set_system_mouse_cursor(A5O_DISPLAY *display,
 ...
 */
static bool show_cursor(A5O_DISPLAY *d)
{
   A5O_DISPLAY_OSX_WIN* dpy = (A5O_DISPLAY_OSX_WIN*) d;
   dpy->show_cursor = YES;
   [NSCursor unhide];
...
static bool hide_cursor(A5O_DISPLAY *d)
{
   A5O_DISPLAY_OSX_WIN* dpy = (A5O_DISPLAY_OSX_WIN*) d;
   dpy->show_cursor = NO;
   [NSCursor hide];
...
static bool acknowledge_resize_display_win_main_thread(A5O_DISPLAY *d)
{
   ASSERT_MAIN_THREAD();
...
/* Call from user thread */
static bool acknowledge_resize_display_win(A5O_DISPLAY *d) {
   ASSERT_USER_THREAD();
   dispatch_sync(dispatch_get_main_queue(), ^{
...
static bool resize_display_win_main_thread(A5O_DISPLAY *d, int w, int h)
{
   ...
   w = _A5O_MAX(w, MINIMUM_WIDTH / scale_factor);
   h = _A5O_MAX(h, MINIMUM_HEIGHT / scale_factor);
...
static bool resize_display_fs(A5O_DISPLAY *d, int w, int h)
{
   A5O_DEBUG("Resize full screen display to %d x %d\n", w, h);
   bool success = true;
   A5O_DISPLAY_OSX_WIN* dpy = (A5O_DISPLAY_OSX_WIN*) d;
#if MAC_OS_X_VERSION_MIN_REQUIRED < 1060
...
static bool is_compatible_bitmap(A5O_DISPLAY* disp, A5O_BITMAP* bmp)
{
   return (_al_get_bitmap_display(bmp) == disp)
      || (((A5O_DISPLAY_OSX_WIN*) _al_get_bitmap_display(bmp))->display_group == ((A5O_DISPLAY_OSX_WIN*) disp)->display_group);
}
 ...
 * Call from user thread.
 */
static void set_window_position(A5O_DISPLAY* display, int x, int y)
{
   ASSERT_USER_THREAD();
   A5O_DISPLAY_OSX_WIN* d = (A5O_DISPLAY_OSX_WIN*) display;
 ...
 * Call from user thread.
 */
static void get_window_position(A5O_DISPLAY* display, int* px, int* py)
{
   ASSERT_USER_THREAD();
   A5O_DISPLAY_OSX_WIN* d = (A5O_DISPLAY_OSX_WIN*) display;
   NSWindow* window = d->win;
...
static bool set_window_constraints(A5O_DISPLAY* display,
...
static bool get_window_constraints(A5O_DISPLAY* display,
   int* min_w, int* min_h, int* max_w, int* max_h)
{
   *min_w = display->min_w;
   *min_h = display->min_h;
 ...
 * Call from user thread.
 */
static void apply_window_constraints(A5O_DISPLAY *display,
   bool onoff)
{
   ASSERT_USER_THREAD();
   A5O_DISPLAY_OSX_WIN* d = (A5O_DISPLAY_OSX_WIN*) display;
 ...
 * Call from user thread
 */
static void set_window_title(A5O_DISPLAY *display, const char *title)
{
   ASSERT_USER_THREAD();
   A5O_DISPLAY_OSX_WIN* dpy = (A5O_DISPLAY_OSX_WIN*) display;
 ...
 * Call from user thread
 */
static void set_icons(A5O_DISPLAY *display, int num_icons, A5O_BITMAP* bitmaps[])
{
   ASSERT_USER_THREAD();
   /* Multiple icons not yet implemented. */
   A5O_BITMAP *bitmap = bitmaps[num_icons - 1];
   NSAutoreleasePool *pool = [[NSAutoreleasePool alloc] init];
   ...
   dispatch_sync(dispatch_get_main_queue(), ^{
      ...
      switch (flag) {
         case A5O_FRAMELESS:
             ...
             */
            A5O_DEBUG("Toggle FRAME for display %p to %d\n", dpy, onoff);
            if (onoff)
            ...
            break;
         case A5O_RESIZABLE:
            A5O_DEBUG("Toggle RESIZABLE for display %p to %d\n", dpy, onoff);
            if (onoff) {
               display->flags |= A5O_RESIZABLE;
               mask |= NSWindowStyleMaskResizable;
            } else {
               display->flags &= ~A5O_RESIZABLE;
               mask &= ~NSWindowStyleMaskResizable;
...
static void acknowledge_drawing_halt(A5O_DISPLAY *display)
{
   A5O_DISPLAY_OSX_WIN *dpy = (A5O_DISPLAY_OSX_WIN *)display;
   al_lock_mutex(dpy->halt_mutex);
...
A5O_DISPLAY_INTERFACE* _al_osx_get_display_driver_win(void)
{
   static A5O_DISPLAY_INTERFACE* vt = NULL;
   if (vt == NULL) {
      vt = al_malloc(sizeof(*vt));
      memset(vt, 0, sizeof(A5O_DISPLAY_INTERFACE));
 ...
 */
NSWindow* al_osx_get_window(A5O_DISPLAY *display)
{
   if (!display)
      return NULL;
   return ((A5O_DISPLAY_OSX_WIN *)display)->win;
}
