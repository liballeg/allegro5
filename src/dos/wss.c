...
static int wss_detect(int input)
{
   ...
   usprintf(wss_desc, uconvert_ascii("Windows Sound System (%d hz) on port %X, using IRQ %d and DMA channel %d", NULL), codec_rates[wss_usedrate].freq, _sound_port, _sound_irq, _sound_dma);
