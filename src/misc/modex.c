...
static BITMAP *modex_init(int w, int h, int v_w, int v_h, int color_depth)
{
   ...
   if (color_depth != 8) {
      ustrcpy(allegro_error, get_config_text("Mode-X only supports 8 bit color"));
      return NULL;
   }
   ...
   if (!mode->regs) {
      ustrcpy(allegro_error, get_config_text("Not a VGA mode-X resolution"));
      return NULL;
   }
...
static BITMAP *xtended_init(int w, int h, int v_w, int v_h, int color_depth)
{
   ...
   /* check it is a valid resolution */
   if (color_depth != 8) {
      ustrcpy(allegro_error, get_config_text("Xtended mode only supports 8 bit color"));
      return NULL;
   }

   if ((w != 640) || (h != 400) || (v_w > 640) || (v_h > 400)) {
      ustrcpy(allegro_error, get_config_text("Xtended mode only supports 640x400"));
      return NULL;
   }
   ...
   if (!addr) {
      ustrcpy(allegro_error, get_config_text("VESA mode 0x100 not available"));
      return NULL;
   }
