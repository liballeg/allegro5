...
{																			\
   ...
   x2 += bmp->x_ofs;														\
																			\
   hooks.draw_rect_##bpp(x1, y, x2, y, color);								\
...
{																			\
   ...
   y2 += bmp->y_ofs;														\
   																			\
   hooks.draw_rect_##bpp(x, y1, x, y2, color);								\
   bmp->id &= ~(BMP_ID_LOCKED | BMP_ID_AUTOLOCK);							\
   release_sem(be_fullscreen_lock);											\
}
...
{																			\
   ...
   y2 += bmp->y_ofs;														\
   																			\
   hooks.draw_rect_##bpp(x1, y1, x2, y2, color);							\
   bmp->id &= ~(BMP_ID_LOCKED | BMP_ID_AUTOLOCK);							\
   release_sem(be_fullscreen_lock);											\
}   
...
static void be_gfx_accel_blit_to_self(BITMAP *source, BITMAP *dest, int source_x, int source_y, int dest_x, int dest_y, int width, int height)
{
   ...
   hooks.blit(source_x, source_y, dest_x, dest_y, width - 1, height - 1);
 ...
 */
void be_gfx_fullscreen_accelerate(int depth)
{
   hooks.draw_line_8   = (LINE8_HOOK)  be_allegro_screen->CardHookAt(LINE8_HOOK_NUM);
   hooks.draw_line_16  = (LINE16_HOOK) be_allegro_screen->CardHookAt(LINE16_HOOK_NUM);
   hooks.draw_line_32  = (LINE32_HOOK) be_allegro_screen->CardHookAt(LINE32_HOOK_NUM);
   hooks.draw_array_8  = (ARRAY8_HOOK) be_allegro_screen->CardHookAt(ARRAY8_HOOK_NUM);
   hooks.draw_array_32 = (ARRAY32_HOOK)be_allegro_screen->CardHookAt(ARRAY32_HOOK_NUM);
   hooks.draw_rect_8   = (RECT8_HOOK)  be_allegro_screen->CardHookAt(RECT8_HOOK_NUM);
   hooks.draw_rect_16  = (RECT16_HOOK) be_allegro_screen->CardHookAt(RECT16_HOOK_NUM);
   hooks.draw_rect_32  = (RECT32_HOOK) be_allegro_screen->CardHookAt(RECT32_HOOK_NUM);
