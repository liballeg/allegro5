...
int main(int argc, char *argv[])
{
   ...
   textprintf_centre(screen, font, SCREEN_W/2, 12, 4, "Sound code test program for Allegro " ALLEGRO_VERSION_STR ", " ALLEGRO_PLATFORM_STR);
   textprintf_centre(screen, font, SCREEN_W/2, 24, 4, "By Shawn Hargreaves, " ALLEGRO_DATE_STR);

   textprintf(screen, font, 0, 84,  2, "Digital sound driver: %s", digi_driver->name);
   textprintf(screen, font, 0, 96,  2, "Description: %s", digi_driver->desc);
   textprintf(screen, font, 0, 108, 2, "Voices: %d", digi_driver->voices);

   textprintf(screen, font, 0, 132, 2, "Midi music driver: %s", midi_driver->name);
   textprintf(screen, font, 0, 144, 2, "Description: %s", midi_driver->desc);
   textprintf(screen, font, 0, 156, 2, "Voices: %d", midi_driver->voices);
   ...
   while ((i < argc) && (item_count < 9)) {
	 ...
	 goto get_out;
      ...
      textprintf(screen, font, 32, 192+item_count*12, 255, "%d: %s", item_count+1, argv[i]);

      item_count++;
      i++;
   }

   textprintf_centre(screen, font, SCREEN_W/2, SCREEN_H-48, 4, "Press a number 1-9 to trigger a sample or midi file");
   textprintf_centre(screen, font, SCREEN_W/2, SCREEN_H-36, 4, "v/V changes sfx volume, p/P changes sfx pan, and f/F changes sfx frequency");
   textprintf_centre(screen, font, SCREEN_W/2, SCREEN_H-24, 4, "space pauses/resumes MIDI playback, and the arrow keys seek through the tune");
   textprintf_centre(screen, font, SCREEN_W/2, SCREEN_H-12, 4, "Use the function keys to doodle a tune");
   ...
   do {
	 ...
	 default:
      ...
      textout_centre(screen, font, buf, SCREEN_W/2, SCREEN_H-120, 255);
