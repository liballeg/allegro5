...
static int option(A5O_CONFIG *config, char *name, int v)
{
   char const *value;
   char str[256];
...
static bool display_warning(A5O_EVENT_QUEUE *queue, A5O_FONT *font)
{
   A5O_EVENT event;
   ...
   for (;;) {
      ...
      al_draw_text(font, white, x, y, A5O_ALIGN_CENTRE,
         "Do not continue if you suffer from photosensitive epilepsy");
      al_draw_text(font, white, x, y + 15, A5O_ALIGN_CENTRE,
         "or simply hate sliding bars.");
      al_draw_text(font, white, x, y + 40, A5O_ALIGN_CENTRE,
         "Press Escape to quit or Enter to continue.");
      
      y += 100;
      al_draw_text(font, white, x, y, A5O_ALIGN_CENTRE, "Parameters from ex_vsync.ini:");
      y += h;
      al_draw_textf(font, white, x, y, A5O_ALIGN_CENTRE, "vsync: %d", vsync);
      y += h;
      al_draw_textf(font, white, x, y, A5O_ALIGN_CENTRE, "fullscreen: %d", fullscreen);
      y += h;
      al_draw_textf(font, white, x, y, A5O_ALIGN_CENTRE, "frequency: %d", frequency);
      y += h;
      al_draw_textf(font, white, x, y, A5O_ALIGN_CENTRE, "bar width: %d", bar_width);
...
int main(int argc, char **argv)
{
   ...
   if (vsync)
      al_set_new_display_option(A5O_VSYNC, vsync, A5O_SUGGEST);
