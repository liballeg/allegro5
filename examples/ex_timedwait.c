...
static bool test_relative_timeout(A5O_FONT *font, A5O_EVENT_QUEUE *queue);
static bool test_absolute_timeout(A5O_FONT *font, A5O_EVENT_QUEUE *queue);
...
static bool test_relative_timeout(A5O_FONT *font, A5O_EVENT_QUEUE *queue)
{
   ...
   while (true) {
      ...
      al_draw_textf(font, al_map_rgb_f(1., 1., 1.), 320, 240, A5O_ALIGN_CENTRE, "Relative timeout (%.2f)", shade);
      al_draw_text(font, al_map_rgb_f(1., 1., 1.), 320, 260, A5O_ALIGN_CENTRE, "Esc to move on, Space to restart.");
      al_flip_display();
   }
}
...
static bool test_absolute_timeout(A5O_FONT *font, A5O_EVENT_QUEUE *queue)
{
   ...
   while (true) {
      ...
      al_draw_textf(font, al_map_rgb_f(1., 1., 1.), 320, 240, A5O_ALIGN_CENTRE, "Absolute timeout (%.2f)", shade);
      al_draw_text(font, al_map_rgb_f(1., 1., 1.), 320, 260, A5O_ALIGN_CENTRE, "Esc to move on, Space to restart.");
      al_flip_display();
   }
}
