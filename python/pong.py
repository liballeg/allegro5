...
class Pallet:
    ...
    def reset(self):
        ...
        (self.x, self.y, self.w, self.h, self.speed, self.color, self.moving) = self.__initials
    ...
    def handle_event(self, ev):
        ...
        if ev.type == ALLEGRO_EVENT_TIMER:
            ...
            if self.difficulty == 1:
                ...
                #Higher difficulty, so we need to precalculate the position of the ball if it is closer than a certain treshold
                if self.ball.xdir == -1 or abs(self.x - self.ball.x) > al_get_display_width(al_get_current_display())*(self.difficulty-1)/self.difficulty:
                    #If the ball is moving away, return to the center of the screen
                    target_y = al_get_display_height(al_get_current_display())/2
                else:
                    #The ball is moving towards this pallet within its FOV.
                    #Calculate what the y location of the ball will be when it lands at this pallets x location
                    target_y = self.ball.y + (self.x - self.ball.x)*self.ball.ydir
                    if target_y < 0:
                        target_y = abs(target_y)
                    elif target_y > al_get_display_height(al_get_current_display()):
                        target_y = al_get_display_height(al_get_current_display()) - target_y % al_get_display_height(al_get_current_display())
...
class Ball:
    ...
    def update(self):
        ...
        if ((new_x <= p.x + p.w and self.xdir == -1) or (new_x + self.size >= p.x and self.xdir == 1)) and new_y + self.size >= p.y and new_y <= p.y + p.h:
            #We hit the pallet
            self.xdir = -self.xdir
            self.speed += 0.1
            self.on_pallet_touch(p)
            self.new_x = self.x
        if self.x < 0 or self.x > al_get_display_width(al_get_current_display()):
            self.on_screen_exit()
...
def main():
    #Initialisation
    difficulty = int(input("What difficulty do you want to play on? Input: 0 for two-player mode, 1-4 for AI difficulty setting.\n"))
    ...
    players = []
    players.append(Player(0,h/2-pallet_h/2,pallet_w,pallet_h,pallet_speed,ALLEGRO_KEY_W,ALLEGRO_KEY_S,al_map_rgb_f(1.0,0.0,0.0)))
    ball = Ball(w/2,h/2,ball_speed,ball_size,players,al_map_rgb_f(0,1.0,0))
    if not difficulty:
        players.append(Player(w-pallet_w,h/2-pallet_h/2, pallet_w, pallet_h, pallet_speed, ALLEGRO_KEY_UP,ALLEGRO_KEY_DOWN,al_map_rgb_f(0.0,0.0,1.0)))
    else:
        players.append(AI(w-pallet_w,h/2-pallet_h/2, pallet_w, pallet_h, pallet_speed,al_map_rgb_f(0.0,0.0,1.0),difficulty,ball))
    
    upgrade_types = [(al_map_rgb_f(0.0,1.0,0.0),lambda pallet:setattr(pallet,'h',pallet.h*1.25)),(al_map_rgb_f(1.0,0.0,0.0),lambda pallet:setattr(pallet,'h',pallet.h * 0.8)),(al_map_rgb_f(1.0,1.0,1.0),lambda pallet:setattr(pallet,'speed',pallet.speed * 1.5)),(al_map_rgb_f(0.3,0.3,0.3),lambda pallet:setattr(pallet,'speed',pallet.speed * 2/3))]
