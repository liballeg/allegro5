...
static int interpret_commandline(struct dat2c* dat2c, int argc, 
    char** argv)
{
    int i = 0;
    ...
    for(i = 1; i < argc; i++) {
        ...
        if(is_arg(argv[i], "-o", "--cfile")) {
            if(++i == argc) {
                fprintf(stderr, "Error: no filename after %s", 
                    argv[i - 1]);
                return 1;
            }
            if(dat2c->fname_c) {
        ...
        if(is_arg(argv[i], "-p", "--prefix")) {
            if(++i == argc) {
                fprintf(stderr, "Error: no name after %s", 
                    argv[i - 1]);
                return 1;
            }
            if(dat2c->prefix) {
        ...
        if(is_arg(argv[i], "-g", "--global")) {
            if(dat2c->global_symbols) {
                fprintf(stderr, "Error: global symbols specified "
                    "twice.\n");
                return 1;
            }
...
{
    /* close files */
    if(dat2c->dat) unload_datafile(dat2c->dat);
    if(dat2c->fp_c) fclose(dat2c->fp_c);
    if(dat2c->fp_h) fclose(dat2c->fp_h);
...
static void _cwrite_esc(FILE* fp, const unsigned char* data, int amt)
{
    ...
    for(rd = data; amt; amt--) {
        ...
        if(was_num_escape) was_num_escape--;
        if(was_trigraph_char) was_trigraph_char--;
        rd++;
    }
    putc('"', fp);
}
    ...
    while(*rd) {
        ...
        if(*rd == '$') {
            if(!strncmp(rd, "$$", 2)) {
                ...
                for(i = 0; i < 4; i++) {
                    if(c[i] == '\'' || c[i] == '\\') putc('\\', fp);
                    if(c[i] < 32) _cwrite_esc_char(fp, c[i]);
                    else putc(c[i], fp);
        ...
        start = rd;
        for(; *rd && *rd != '$'; rd++);
        if(rd - start) fwrite(start, 1, rd - start, fp);
    }
...
static const char* get_c_identifier(const char* prefix, DATAFILE* obj)
{
    ...
    for(iter = first_cident; iter; iter = iter->next)
        if(iter->obj == obj) return iter->name;
...
static void write_source_end(struct dat2c* dat2c, const char* dat_name)
{
    ...
    cwrite(dat2c, C, "/* end of $string$ */$n$"
                     "$n$$n$$n$",
        dat2c->fname_c);
}
    ...
    const char* name, int export)
{
    ...
    cwrite(dat2c, C, "$string$struct { int w, h; int clip; int cl, cr, ct, cb;\n"
                     ...
                     "    0, 0, /* write_bank, read_bank */$n$"
                     "    $string lower$_lines, /* data */$n$"
                     "    0, 0, 0, 0, 0,$n$"
                     "    { /* line[] array */$n$",
        export ? "" : "static ",
...
static int cvt_BITMAP(struct dat2c* dat2c, DATAFILE* dat,
    const char* name)
{
    int result = 0;
    result = cvt_BITMAP_aux(dat2c, dat->dat, name, 
        dat2c->global_symbols);
...
static int cvt_FONT(struct dat2c* dat2c, DATAFILE* dat, const char* name)
{
    ...
    cwrite(dat2c, C, "$string$FONT $string lower$ = {$n$"
        ...
        name, (num_ranges - 1),
        f->height,
        f->vtable == font_vtable_color ? 1 : 0);
 ...
 */
static int cvt_SAMPLE(struct dat2c* dat2c, DATAFILE* dat, 
    const char* name)
{
    SAMPLE* spl = dat->dat;
    ...
    cwrite(dat2c, C, "$string$SAMPLE $string lower$ = {$n$"
        ...
        spl->len * (spl->bits / 8) * (spl->stereo ? 2 : 1),
            4, spl->data);
...
static int cvt_MIDI(struct dat2c* dat2c, DATAFILE* dat, const char* name)
{
    ...
    for(i = 0; i < MIDI_TRACKS; i++) {
        ...
        if(i < (MIDI_TRACKS - 1)) cwrite(dat2c, C, ",$n$");
        else cwrite(dat2c, C, "$n$");
    }
    
    cwrite(dat2c, C, "    } /* end of tracks */$n$"
                     "};$n$"
                     "$n$$n$$n$");
    ...
    const char* name)
{
    ...
    /* declare this in the header if we are exporting objects */
    if(dat2c->global_symbols) {
        cwrite(dat2c, H, "extern RLE_SPRITE $string lower$;$n$$n$", 
            name);
    }
 ...
 */
static int cvt_CSPRITE(struct dat2c* dat2c, DATAFILE* dat, 
    const char* name)
{
    if(!dat2c->convert_compiled_sprites) {
 ...
 *  Writes a palette.
 */
static int cvt_PALETTE(struct dat2c* dat2c, DATAFILE* dat, 
    const char* name)
{
    RGB* pal = dat->dat;
    int i = 0;
 ...
 *  Returns 0 on success.
 */
static int write_datafile(struct dat2c* dat2c, DATAFILE* dat, 
    const char* prefix, const char* datafile_name)
{
    ...
    for(i = 0; dat[i].type != DAT_END; i++) {
        ...
        for(j = 0; dat2c_converters[j].type != DAT_END; j++)
            if(dat2c_converters[j].type == dat[i].type) break;

        /* skip DAT_PROPERTY/DAT_INFO */
        if(!dat2c_converters[j].cvt) continue;
    ...
    cwrite(dat2c, C, "    { 0, DAT_END, 0, 0 }$n$"
                     "};$n$"
                     "$n$$n$$n$");
...
int do_conversion(struct dat2c* dat2c)
{
    ...
    result = write_datafile(dat2c, dat2c->dat, 
        dat2c->prefix ? dat2c->prefix : "", prefixed_name);
