...
{
   A5O_ERROR("%s (error code: %d)\n", string, error);
}

A5O_AUDIO_DRIVER *_al_kcm_driver = NULL;

#if defined(A5O_CFG_KCM_OPENAL)
   extern struct A5O_AUDIO_DRIVER _al_kcm_openal_driver;
#endif
#if defined(A5O_CFG_KCM_OPENSL)
   extern struct A5O_AUDIO_DRIVER _al_kcm_opensl_driver;
#endif
#if defined(A5O_CFG_KCM_ALSA)
   extern struct A5O_AUDIO_DRIVER _al_kcm_alsa_driver;
#endif
#if defined(A5O_CFG_KCM_OSS)
   extern struct A5O_AUDIO_DRIVER _al_kcm_oss_driver;
#endif
#if defined(A5O_CFG_KCM_DSOUND)
   extern struct A5O_AUDIO_DRIVER _al_kcm_dsound_driver;
#endif
#if defined(A5O_CFG_KCM_AQUEUE)
   extern struct A5O_AUDIO_DRIVER _al_kcm_aqueue_driver;
#endif
#if defined(A5O_CFG_KCM_PULSEAUDIO)
   extern struct A5O_AUDIO_DRIVER _al_kcm_pulseaudio_driver;
#endif
#if defined(A5O_SDL)
   extern struct A5O_AUDIO_DRIVER _al_kcm_sdl_driver;
#endif
 ...
 */
size_t al_get_channel_count(A5O_CHANNEL_CONF conf)
{
 ...
 */
size_t al_get_audio_depth_size(A5O_AUDIO_DEPTH depth)
{
   switch (depth) {
...
A5O_CHANNEL_CONF _al_count_to_channel_conf(int num_channels)
{
   switch (num_channels) {
      case 1:
         return A5O_CHANNEL_CONF_1;
      case 2:
         return A5O_CHANNEL_CONF_2;
      case 3:
         return A5O_CHANNEL_CONF_3;
      case 4:
         return A5O_CHANNEL_CONF_4;
      case 6:
         return A5O_CHANNEL_CONF_5_1;
      case 7:
         return A5O_CHANNEL_CONF_6_1;
      case 8:
         return A5O_CHANNEL_CONF_7_1;
      default:
         return 0;
   }
}
...
A5O_AUDIO_DEPTH _al_word_size_to_depth_conf(int word_size)
{
   switch (word_size) {
      case 1:
...
static A5O_AUDIO_DRIVER_ENUM get_config_audio_driver(void)
{
   ...
   if (!value || value[0] == '\0')
      return A5O_AUDIO_DRIVER_AUTODETECT;

   if (0 == _al_stricmp(value, "ALSA"))
      return A5O_AUDIO_DRIVER_ALSA;

   if (0 == _al_stricmp(value, "OPENAL"))
      return A5O_AUDIO_DRIVER_OPENAL;
   
   if (0 == _al_stricmp(value, "OPENSL"))
      return A5O_AUDIO_DRIVER_OPENSL;

   if (0 == _al_stricmp(value, "OSS"))
      return A5O_AUDIO_DRIVER_OSS;

   if (0 == _al_stricmp(value, "PULSEAUDIO"))
      return A5O_AUDIO_DRIVER_PULSEAUDIO;

   if (0 == _al_stricmp(value, "DSOUND") || 0 == _al_stricmp(value, "DIRECTSOUND"))
      return A5O_AUDIO_DRIVER_DSOUND;
 ...
 */
const A5O_AUDIO_DEVICE* al_get_audio_output_device(int index)
{
   if (_al_kcm_driver) {
      if (_al_kcm_driver->get_output_devices) {
         _AL_LIST* audio_devices = _al_kcm_driver->get_output_devices();
 ...
 */
const char* al_get_audio_device_name(const A5O_AUDIO_DEVICE * device)
{
   return device ? device->name : NULL;
}

static bool do_install_audio(A5O_AUDIO_DRIVER_ENUM mode)
{
   bool retVal;

   /* check to see if a driver is already installed and running */
   if (_al_kcm_driver) {
      _al_set_error(A5O_GENERIC_ERROR, "A driver already running");
      return false;
   }

   if (mode == A5O_AUDIO_DRIVER_AUTODETECT) {
      mode = get_config_audio_driver();
   }

   switch (mode) {
      case A5O_AUDIO_DRIVER_AUTODETECT:
#if defined(A5O_SDL)
         retVal = do_install_audio(A5O_AUDIO_DRIVER_SDL);
         if (retVal)
            return retVal;
#endif
#if defined(A5O_CFG_KCM_AQUEUE)
         retVal = do_install_audio(A5O_AUDIO_DRIVER_AQUEUE);
         if (retVal)
            return retVal;
#endif
 ...
 */
#if defined(A5O_CFG_KCM_PULSEAUDIO)
         retVal = do_install_audio(A5O_AUDIO_DRIVER_PULSEAUDIO);
         if (retVal)
            return retVal;
#endif
#if defined(A5O_CFG_KCM_ALSA)
         retVal = do_install_audio(A5O_AUDIO_DRIVER_ALSA);
         if (retVal)
            return retVal;
#endif
#if defined(A5O_CFG_KCM_DSOUND)
         retVal = do_install_audio(A5O_AUDIO_DRIVER_DSOUND);
         if (retVal)
            return retVal;
#endif
#if defined(A5O_CFG_KCM_OSS)
         retVal = do_install_audio(A5O_AUDIO_DRIVER_OSS);
         if (retVal)
            return retVal;
#endif
#if defined(A5O_CFG_KCM_OPENAL)
         retVal = do_install_audio(A5O_AUDIO_DRIVER_OPENAL);
         if (retVal)
            return retVal;
#endif
#if defined(A5O_CFG_KCM_OPENSL)
         retVal = do_install_audio(A5O_AUDIO_DRIVER_OPENSL);
         if (retVal)
            return retVal;
#endif

         _al_set_error(A5O_INVALID_PARAM, "No audio driver can be used.");
         _al_kcm_driver = NULL;
         return false;

      case A5O_AUDIO_DRIVER_AQUEUE:
         #if defined(A5O_CFG_KCM_AQUEUE)
            ...
            return false;
         #else
            _al_set_error(A5O_INVALID_PARAM, "Audio Queue driver not available on this platform");
            return false;
         #endif

      case A5O_AUDIO_DRIVER_OPENAL:
         #if defined(A5O_CFG_KCM_OPENAL)
            ...
            return false;
         #else
            _al_set_error(A5O_INVALID_PARAM, "OpenAL not available on this platform");
            return false;
         #endif

      case A5O_AUDIO_DRIVER_OPENSL:
         #if defined(A5O_CFG_KCM_OPENSL)
            ...
            return false;
         #else
            _al_set_error(A5O_INVALID_PARAM, "OpenSL not available on this platform");
            return false;
         #endif

      case A5O_AUDIO_DRIVER_ALSA:
         #if defined(A5O_CFG_KCM_ALSA)
            ...
            return false;
         #else
            _al_set_error(A5O_INVALID_PARAM, "ALSA not available on this platform");
            return false;
         #endif

      case A5O_AUDIO_DRIVER_OSS:
         #if defined(A5O_CFG_KCM_OSS)
            ...
            return false;
         #else
            _al_set_error(A5O_INVALID_PARAM, "OSS not available on this platform");
            return false;
         #endif

      case A5O_AUDIO_DRIVER_PULSEAUDIO:
         #if defined(A5O_CFG_KCM_PULSEAUDIO)
            ...
            return false;
         #else
            _al_set_error(A5O_INVALID_PARAM, "PulseAudio not available on this platform");
            return false;
         #endif

      case A5O_AUDIO_DRIVER_DSOUND:
         #if defined(A5O_CFG_KCM_DSOUND)
            ...
            return false;
         #else
            _al_set_error(A5O_INVALID_PARAM, "DirectSound not available on this platform");
            return false;
         #endif

      case A5O_AUDIO_DRIVER_SDL:
         #if defined(A5O_SDL)
            ...
            return false;
         #else
            _al_set_error(A5O_INVALID_PARAM, "SDL not available on this platform");
            return false;
         #endif

      default:
         _al_set_error(A5O_INVALID_PARAM, "Invalid audio driver");
         return false;
   }
}
...
bool al_install_audio(void)
{
   ...
   ret = do_install_audio(A5O_AUDIO_DRIVER_AUTODETECT);
   return ret;
}
