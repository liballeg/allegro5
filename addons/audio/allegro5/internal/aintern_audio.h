...
typedef enum A5O_AUDIO_DRIVER_ENUM
{
   /* Various driver modes. */
   A5O_AUDIO_DRIVER_AUTODETECT = 0x20000,
   A5O_AUDIO_DRIVER_OPENAL     = 0x20001,
   A5O_AUDIO_DRIVER_ALSA       = 0x20002,
   A5O_AUDIO_DRIVER_DSOUND     = 0x20003,
   A5O_AUDIO_DRIVER_OSS        = 0x20004,
   A5O_AUDIO_DRIVER_AQUEUE     = 0x20005,
   A5O_AUDIO_DRIVER_PULSEAUDIO = 0x20006,
   A5O_AUDIO_DRIVER_OPENSL     = 0x20007,
   A5O_AUDIO_DRIVER_SDL        = 0x20008
} A5O_AUDIO_DRIVER_ENUM;

typedef struct A5O_AUDIO_DRIVER A5O_AUDIO_DRIVER;
struct A5O_AUDIO_DRIVER {
   const char     *specifier;

   int            (*open)(void);
   void           (*close)(void);

   int            (*allocate_voice)(A5O_VOICE*);
   void           (*deallocate_voice)(A5O_VOICE*);

   int            (*load_voice)(A5O_VOICE*, const void*);
   void           (*unload_voice)(A5O_VOICE*);

   int            (*start_voice)(A5O_VOICE*);
   int            (*stop_voice)(A5O_VOICE*);

   bool           (*voice_is_playing)(const A5O_VOICE*);

   unsigned int   (*get_voice_position)(const A5O_VOICE*);
   int            (*set_voice_position)(A5O_VOICE*, unsigned int);
   
   
   int            (*allocate_recorder)(struct A5O_AUDIO_RECORDER *);
   void           (*deallocate_recorder)(struct A5O_AUDIO_RECORDER *);

   _AL_LIST*      (*get_output_devices)(void);
};

extern A5O_AUDIO_DRIVER *_al_kcm_driver;
 ...
 * would be one A5O_VOICE per system/hardware voice.
 */
struct A5O_VOICE {
   A5O_AUDIO_DEPTH  depth;
   A5O_CHANNEL_CONF chan_conf;
   ...
   A5O_SAMPLE_INSTANCE       *attached_stream;
                        /* The stream that is attached to the voice, or NULL.
                         * May be an A5O_SAMPLE_INSTANCE or A5O_MIXER object.
                         */
...
typedef void (*stream_reader_t)(void *source, void **vbuf,
   unsigned int *samples, A5O_AUDIO_DEPTH buffer_depth, size_t dest_maxc);
...
/* The sample struct also serves the base of A5O_AUDIO_STREAM, A5O_MIXER. */
struct A5O_SAMPLE_INSTANCE {
   /* A5O_SAMPLE_INSTANCE does not generate any events yet but A5O_AUDIO_STREAM
    * does, which can inherit only A5O_SAMPLE_INSTANCE. */
   A5O_EVENT_SOURCE es;
...
void _al_kcm_destroy_sample(A5O_SAMPLE_INSTANCE *sample, bool unregister);
...
typedef size_t (*stream_callback_t)(A5O_AUDIO_STREAM *, void *, size_t);
typedef void (*unload_feeder_t)(A5O_AUDIO_STREAM *);
typedef bool (*rewind_feeder_t)(A5O_AUDIO_STREAM *);
typedef bool (*seek_feeder_t)(A5O_AUDIO_STREAM *, double);
typedef double (*get_feeder_position_t)(A5O_AUDIO_STREAM *);
typedef double (*get_feeder_length_t)(A5O_AUDIO_STREAM *);
typedef bool (*set_feeder_loop_t)(A5O_AUDIO_STREAM *, double, double);

struct A5O_AUDIO_STREAM {
   A5O_SAMPLE_INSTANCE spl;
                        /* A5O_AUDIO_STREAM is derived from
                         * A5O_SAMPLE_INSTANCE.
                         */
   ...
   A5O_THREAD        *feed_thread;
   A5O_MUTEX         *feed_thread_started_mutex;
   A5O_COND          *feed_thread_started_cond;
   ...
   stream_callback_t     feeder;
                         /* If A5O_AUDIO_STREAM has been created by
...
/* A5O_MIXER is derived from A5O_SAMPLE_INSTANCE. Certain internal functions and
 ...
 */
struct A5O_MIXER {
   A5O_SAMPLE_INSTANCE          ss;
                           /* A5O_MIXER is derived from A5O_SAMPLE_INSTANCE. */

   A5O_MIXER_QUALITY   quality;

   postprocess_callback_t  postprocess_callback;
   void                    *pp_callback_userdata;

   _AL_VECTOR              streams;
                           /* Vector of A5O_SAMPLE_INSTANCE*.  Holds the list of
                            * streams being mixed together.
                            */
   _AL_LIST_ITEM           *dtor_item;
};

extern void _al_kcm_mixer_rejig_sample_matrix(A5O_MIXER *mixer,
   A5O_SAMPLE_INSTANCE *spl);
...
typedef enum {
   A5O_NO_ERROR       = 0,
   A5O_INVALID_PARAM  = 1,
   A5O_INVALID_OBJECT = 2,
   A5O_GENERIC_ERROR  = 255
} AL_ERROR_ENUM;

extern void _al_set_error(int error, char* string);

struct A5O_AUDIO_DEVICE {
   char* name;
   void* identifier;
};

/* Supposedly internal */
A5O_KCM_AUDIO_FUNC(void*, _al_kcm_feed_stream, (A5O_THREAD *self, void *vstream));
...
A5O_KCM_AUDIO_FUNC(void, _al_kcm_shutdown_default_mixer, (void));

A5O_KCM_AUDIO_FUNC(A5O_CHANNEL_CONF, _al_count_to_channel_conf, (int num_channels));
A5O_KCM_AUDIO_FUNC(A5O_AUDIO_DEPTH, _al_word_size_to_depth_conf, (int word_size));

A5O_KCM_AUDIO_FUNC(void, _al_emit_audio_event, (int event_type));
