...
static A5O_MUTEX* global_mutex;

static A5O_COND* wm_size_cond;
 ...
 */
static A5O_PATH* _tcreate_path(const TCHAR* ts)
{
   char* tmp = _twin_tchar_to_utf8(ts);
   A5O_PATH* path = al_create_path(tmp);
   al_free(tmp);
   return path;
}
/* tcreate_path:
 ...
 */
static A5O_PATH* _tcreate_path_for_directory(const TCHAR* ts)
{
   char* tmp = _twin_tchar_to_utf8(ts);
   A5O_PATH* path = al_create_path_for_directory(tmp);
   al_free(tmp);
   return path;
}
...
static TCHAR* _extract_dirname(A5O_PATH *fullpath)
{
   TCHAR* wpath = NULL;
   bool is_dir;
   const A5O_USTR *path = al_path_ustr(fullpath, A5O_NATIVE_PATH_SEP);

   if (al_filename_exists(al_cstr(path))) {
      A5O_FS_ENTRY *fs = al_create_fs_entry(al_cstr(path));
   ...
   if (is_dir) {
      ...
      A5O_PATH* initial_dir_path = NULL;
      initial_dir_path = al_clone_path(fullpath);
      if (initial_dir_path) {
         al_set_path_filename(initial_dir_path, NULL);
         wpath = _twin_utf8_to_tchar(al_path_cstr(initial_dir_path, A5O_NATIVE_PATH_SEP));
...
static bool select_folder(A5O_DISPLAY_WIN *win_display,
...
static A5O_USTR *create_filter_string(const _AL_VECTOR *patterns)
{
   A5O_USTR *filter = al_ustr_new("");
   ...
   A5O_NATIVE_DIALOG *fd)
{
   ...
   TCHAR* wfilter = NULL;
   TCHAR* wpath = NULL;
   A5O_USTR *filter_string = NULL;

   buf[0] = '\0';

   win_display = (A5O_DISPLAY_WIN *)display;

   if (fd->flags & A5O_FILECHOOSER_FOLDER) {
   ...
   if (fd->flags & A5O_FILECHOOSER_SAVE) {
      flags |= OFN_OVERWRITEPROMPT;
   }
   else {
      flags |= (fd->flags & A5O_FILECHOOSER_FILE_MUST_EXIST) ? OFN_FILEMUSTEXIST : 0;
   }
   flags |= (fd->flags & A5O_FILECHOOSER_MULTIPLE) ? OFN_ALLOWMULTISELECT : 0;
   flags |= (fd->flags & A5O_FILECHOOSER_SHOW_HIDDEN) ? 0x10000000 : 0; // NOTE: 0x10000000 is FORCESHOWHIDDEN
   ofn.Flags = flags;
   ...
   if (!ret) {
      ...
      if (err != ERROR_SUCCESS) {
         A5O_ERROR("al_show_native_file_dialog failed: %s\n", _al_win_error(err));
      }
      return false;
   }
   ...
   A5O_NATIVE_DIALOG *fd)
{
   ...
   if (fd->flags & A5O_MESSAGEBOX_QUESTION)
      type |= MB_ICONQUESTION;
   else if (fd->flags & A5O_MESSAGEBOX_WARN)
      type |= MB_ICONWARNING;
   else if (fd->flags & A5O_MESSAGEBOX_ERROR)
      type |= MB_ICONERROR;
   else
      type |= MB_ICONINFORMATION;

   if (fd->flags & A5O_MESSAGEBOX_YES_NO)
      type |= MB_YESNO;
   else if (fd->flags & A5O_MESSAGEBOX_OK_CANCEL)
      type |= MB_OKCANCEL;
...
/* Emit close event. */
static void wlog_emit_close_event(A5O_NATIVE_DIALOG *textlog, bool keypress)
{
   A5O_EVENT event;
 ...
 */
static A5O_USTR* convert_crlf(A5O_USTR* s) {
   int pos = 0;
   int ch, prev;
...
static void wlog_do_append_native_text_log(A5O_NATIVE_DIALOG *textlog)
{
   int index;
...
static LRESULT CALLBACK wlog_text_log_callback(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
   ...
   A5O_NATIVE_DIALOG* textlog = (A5O_NATIVE_DIALOG*)GetWindowLongPtr(hWnd, GWLP_USERDATA);
...
static bool open_native_text_log_inner(A5O_NATIVE_DIALOG *textlog)
{
   LPCTSTR font_name;
   HWND hWnd;
   HWND hLog;
...
bool _al_open_native_text_log(A5O_NATIVE_DIALOG *textlog)
{
   ...
   if (wlog_count == 0) {
      A5O_DEBUG("Load rich edit module\n");
      A5O_ASSERT(!wlog_rich_edit_module);
...
static bool menu_callback(A5O_DISPLAY *display, UINT msg, WPARAM wParam, LPARAM lParam,
   ...
   if (msg == WM_COMMAND && lParam == 0) {
      ...
      if (menu_id) {
         ...
         if (_al_find_menu_item_unique(menu_id->menu, id, NULL, &index)) {
            if (al_get_menu_item_flags(menu_id->menu, -index) & A5O_MENU_ITEM_CHECKBOX) {
               /* Toggle the checkbox, since Windows doesn't do that automatically. */
               al_toggle_menu_item_flags(menu_id->menu, -index, A5O_MENU_ITEM_CHECKED);
            }
         }
      }
...
static void init_menu_info(MENUITEMINFO *info, A5O_MENU_ITEM *menu)
{
   ...
   if (menu->flags & A5O_MENU_ITEM_DISABLED) {
      info->fState |= MFS_DISABLED;
   }

   if (menu->icon) {
      /* convert A5O_BITMAP to HBITMAP (could be moved into a public function) */
      ...
      lock = al_lock_bitmap(menu->icon, A5O_PIXEL_FORMAT_ARGB_8888, A5O_LOCK_READONLY);
